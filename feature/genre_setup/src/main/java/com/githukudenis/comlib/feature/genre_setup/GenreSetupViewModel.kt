                ...
                when (val genres = genresRepository.getGenres()) {
                    ...
                    is ResponseResult.Success -> {
                        update {
                            copy(
                                isLoading = false,
                                genres = genres.data.data.genres.take(10).map { SelectableGenreItem(genre = it) }
                            )
                        }
                    }
                }
            ...
            when (result) {
                ...
                is ResponseResult.Success -> {
                    userRepository.updateUser(
                        result.data.data.user.copy(
                            preferredGenres = state.value.genres.filter { it.isSelected }.map { it.genre.id }
                        )
                    )
                }
            }
